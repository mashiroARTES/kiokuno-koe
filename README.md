...
| LoRA 学習支援 | Gemma 4 26B A4B 向け QLoRA スクリプト自動生成 |
...
### LoRA ファインチューニング支援
- 会話履歴・記憶データから JSONL データセット自動生成（`/api/finetune/export/:characterId`）
- Gemma 4 26B A4B 向け QLoRA 学習 Python スクリプト自動生成（`/api/finetune/script/:characterId`）
- 推論スクリプト自動生成（`/api/finetune/inference-script/:characterId`）
...
### LoRA ファインチューニング（`/api/finetune`）

| メソッド | パス | 説明 |
|---|---|---|
| GET | `/api/finetune/export/:characterId` | JSONL データセットをダウンロード |
| GET | `/api/finetune/script/:characterId` | QLoRA 学習スクリプト（.py）をダウンロード |
| GET | `/api/finetune/inference-script/:characterId` | 推論スクリプト（.py）をダウンロード |
...
| model_adapter_path | TEXT | LoRA アダプタパス（将来用） |
...
2. 「JSONL エクスポート」で会話・記憶データをダウンロード
3. 「学習スクリプト生成」で Python スクリプトをダウンロード
4. ローカル GPU 環境で実行:
